...
全 Todo 完了
- T1 ✓ GET / の初期フォーム表示を実装
- T2 ✓ 入力行の動的追加を JavaScript で実装
- T3 ✓ 早番・遅番希望の不正値チェックを実装（InvalidWishError）
- T4 ✓ 氏名重複チェックの結果を画面に反映
- T5 ✓ シフト算出（assign の呼び出し）を実装
- T6 ✓ 成立時の結果を表形式で表示
- T7 ✓ 不成立時のメッセージを表示
- T8 ✓ 全体テストと画面表示を確認
...
- `./mvnw test`：Tests run: 28, Failures: 0, Errors: 0, Skipped: 0（成功）
- Spotless チェック：違反なし
- Checkstyle チェック：違反なし
- ブラウザ確認：入力欄・「行を追加」「シフトを作成」ボタン確認済み
