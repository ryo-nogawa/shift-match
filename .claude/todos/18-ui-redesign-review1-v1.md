...
- T6 実行結果：`./mvnw spotless:apply` 実行後、`./mvnw test` で 68 件成功（Checkstyle 違反 0 件、Spotless 違反なし）。`git status` で作業ツリークリーン。`git diff main --stat -- src/main/java pom.xml` で差分なし。
