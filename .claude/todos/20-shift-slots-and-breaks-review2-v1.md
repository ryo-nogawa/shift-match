...
- [ ] **Q3. 結果表のテストの期待値を仕様どおりに設定する（F-4、レビュー SHOULD）**
  ...
  - 依頼事項（差し戻し・追記）：現在のテスト `displaysCorrectNumberOfRowsAndCorrectWorkSchedules` は、次の 3 点で完了条件を満たしていない。直すこと
    ...
    2. 8 行の検証が、最初の `<tbody>`（入力表 `id="employee-rows"`）を対象にしている。`result-table` の `tbody` を対象にする
    3. 行ごと・枠順の検証がない。結果表の 8 行を、上から順に取り出し、各行に「その行の氏名・勤務時間・休憩時間」が、この順で含まれることを 8 行すべてで検証する（氏名は `A`〜`H` の入力順で、8 名全員が全枠 ○ のとき、入力順どおり枠 1→6 に割り当てられる。実サービスではなくモックの戻り値を使っている場合は、モックの戻り値の氏名と枠に合わせる）
  - 追加の完了条件：
    - 変異確認：`index.html` の結果表の休憩時間の出力を一時的に壊す（例：休憩開始の時刻の `th:text` を空にする）と、このテストが失敗することを実際に確認し、確認後は必ず元に戻す（`git diff` で `index.html` に差分が残っていないことを確認する。実行ログに、壊した箇所と、失敗したことを書く）
    - 勤務時間についても同様に、結果表の勤務時間の出力を壊すと失敗することを確認する
