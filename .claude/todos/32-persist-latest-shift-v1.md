...
- [x] **T1. H2・JDBC の依存と接続設定、スキーマを追加する**
...
- [x] **T3. 2 回目の保存は最新データで上書きされる**
  ...
  - 完了条件：
    ...
    - `./mvnw test -Dtest=LatestShiftRepositoryTest` が成功する
- [x] **T4. 決定したシフト（割り当てとスコア）を保存し、不成立のときは消す**
...
- [x] **T7. 保存に失敗したらエラーとリトライを促す文言を表示する**
...
- 2026-09-26 メインエージェント：implementer の報告は【完了】（T1〜T8 を個別コミット、`./mvnw test` 191 件成功）だが Todo のチェックが未反映だったため、コミット履歴と全テスト結果を確認したうえで T1〜T8 にチェックを付けた。T1 は初回、`pom.xml` の編集がガードで拒否され未完了（ガードを ask に緩和後に再依頼して完了）。
