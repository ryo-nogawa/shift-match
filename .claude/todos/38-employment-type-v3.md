...
- 前回（v2）の失敗理由：T8 で結果表に「区分」列を追加したところ、既存テスト `ShiftControllerTest#showsWishRangeWithoutGapAndSlotTags` が「1 行のセル数は 4」を期待していたため失敗し、`implementer` がテストと実装をロールバックして【未完了】と報告した。T5〜T7 は完了（54f6ea7・263a662・062bdf5）
- 今回（v3）の変更点：T8 に具体的な手順を追記した。結果表に列を足すのは仕様変更（要件定義 7 章）による正当な変更なので、この既存テストの期待値は 4 → 5 に更新してよい（テストの期待値を仕様と異なる値に書き換える行為にはあたらない）。ただし他の検証内容（希望時間帯の表示、`slot-tag` が無いこと）は弱めない
...
- [x] **T5. フォームで区分を受け取り、`Employee` に変換する（初期値は常勤）**（完了・54f6ea7）
- [x] **T6. 入力チェック V-7（区分が 3 択以外ならエラー）を追加する**（完了・263a662）
- [x] **T7. 入力画面に「区分」列を追加する**（完了・062bdf5）
- [x] **T8. 割当結果画面に区分を表示する（F-4）**
  - 手順のヒント：先に「結果表の各行に区分ラベルが含まれる」テストを書いて RED を確認 → `index.html` の結果表に「区分」列（`assignment.employee().employmentType().label()`）を追加 → 既存テスト `showsWishRangeWithoutGapAndSlotTags` の「4 cells」を「5 cells」に更新（他の検証は弱めない）→ 未出勤者のチップに `（ラベル）` を併記するテストと実装を追加する。列の増減で他のテストが失敗した場合も、列数の期待値だけを更新する
...
- [x] **T9. 仕様書・README・AGENTS.md を更新する**
