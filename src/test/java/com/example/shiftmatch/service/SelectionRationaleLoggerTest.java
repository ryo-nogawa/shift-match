...
class SelectionRationaleLoggerTest {
  ...
  class 正常系 {
    ...
    @Test
    @DisplayName("[7.2 節] Given: 成立日のとき, When: log を実行すると, Then: 各ログ行に日付が含まれる")
    void logIncludesDateForSuccessfulDay() {
    ...
    @Test
    @DisplayName("[7.2 節] Given: 不成立日のとき, When: log を実行すると, Then: 日付と勤務できる人数が含まれる")
    void logIncludesDateAndAvailableCountForUnsuccessfulDay() {
    ...
    @Test
    @DisplayName("[7.2 節] Given: 氏名に改行を含むとき, When: log を実行すると, Then: 改行が \\n に変換され偽のログ行が作られない")
    ...
    @Test
    @DisplayName("[7.2 節] Given: 異なる 2 日のログが出力されるとき, When: ログを確認すると, Then: 日付で区別できる")
    void differentDatesDistinguishable() {
